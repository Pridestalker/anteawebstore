...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _common_navigation__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./common/navigation */ "./assets/scripts/routes/common/navigation.js");

/* harmony default export */ __webpack_exports__["default"] = ({
  init: function init() {
    // Javascript that fires on all pages.
    Object(_common_navigation__WEBPACK_IMPORTED_MODULE_0__["navHandler"])();
  },
...
/***/ "./assets/scripts/routes/common/navigation.js":
/*!****************************************************!*\
  !*** ./assets/scripts/routes/common/navigation.js ***!
  \****************************************************/
...
var navHandler = function navHandler() {
  ...
  try {
    for (var _iterator = hovers[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      var el = _step.value;
...
var _navHandler = function _navHandler(element) {
  var _element$dataset = element.dataset,
      target = _element$dataset.target,
      event = _element$dataset.event;
  var acceptedEvents = ['mouseover', 'click'];
  ...
  if (acceptedEvents.indexOf(event) === -1) {
    throw Error("Incorrect event used, supported events: ".concat(acceptedEvents.join(', ')));
  }
...
var _addEvents = function _addEvents(_ref) {
  var element = _ref.element,
      event = _ref.event,
      target = _ref.target;
  var el = document.querySelector(target);
  var elClasses = ['active'];
  var bodyClasses = ['hovering', "hover-".concat(target.split('.').join('').split('#').join().split('-').join('_')), "hover-".concat(element.id.split('.').join('').split('#').join().split('-').join('_'))];
  element.addEventListener(event, function () {
    if (el.length === 0) {
      return;
    }

    for (var _i = 0, _elClasses = elClasses; _i < _elClasses.length; _i++) {
      var c = _elClasses[_i];
      el.classList.toggle(c);
    }

    for (var _i2 = 0, _bodyClasses = bodyClasses; _i2 < _bodyClasses.length; _i2++) {
      var _c = _bodyClasses[_i2];
      document.body.classList.toggle(_c);
    }
  });

  if (event === 'mouseover') {
    element.addEventListener('mouseout', function () {
      var _el$classList, _document$body$classL;

      if (el.length === 0) {
        return;
      }

      (_el$classList = el.classList).remove.apply(_el$classList, elClasses);
...
__webpack_require__.r(__webpack_exports__);
/* harmony default export */ __webpack_exports__["default"] = ({
  init: function init() {// Javascript that fires on home.
  },
...
var urlSwitcher = function urlSwitcher(attr, val) {
  ...
  qp.set(attr, val); // window.location.search = qp.toString();
};
